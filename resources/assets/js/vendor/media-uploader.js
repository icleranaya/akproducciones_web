jQuery(document).ready(function($){
		...
		if ( optionsframework_upload ) {
			...
			optionsframework_upload.on( 'select', function() {
				...
				if ( attachment.attributes.type == 'image' ) {
					optionsframework_selector.find('.screenshot').empty().hide().append('<img src="' + attachment.attributes.url + '"><a class="remove-image">Eliminar</a>').slideDown('fast');
				}
				optionsframework_selector.find('.upload-button').unbind().addClass('remove-file').removeClass('upload-button').val(lb_l10n.remove);
	...
	function optionsframework_remove_file(selector) {
		...
		selector.find('.remove-file').unbind().addClass('upload-button').removeClass('remove-file').val(lb_l10n.upload);
