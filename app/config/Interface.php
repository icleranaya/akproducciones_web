	...
	static function optionsframework_tabs() {
		...
		foreach ( $options as $value ) {
			...
			if ( $value['type'] == "heading" ) {
				...
				$menu .= '<li class="tab-link">';
				$menu .= '<a id="tab'.  $counter . '-tab" class="sm-txt-bold ' . $class .'" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#tab'.  $counter ) . '">' . esc_html( $value['name'] ) . ' ';
				$menu .= '<svg class="svg-arrow-right" data-src="' . get_template_directory_uri() . '/public/images/icons/arrow_down.svg"></svg>';
				$menu .= '</a>';
				$menu .= '</li>';
			}
		}
	...
	static function optionsframework_fields() {
		...
		foreach ( $options as $value ) {
			...
			if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) ) {
				...
				if ( isset( $value['name'] ) ) {
					$output .= '<h6 class="heading">' . esc_html( $value['name'] ) . '</h6>' . "\n";
				}
			...
			switch ( $value['type'] ) {
				...
				// Basic text input
				case 'text':
					$output .= '<label for="' . esc_attr( $value['id'] ) . '" class="input-container">';
					$output .= '<input type="text" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" id="' . esc_attr( $value['id'] ) . '" value="' . esc_attr( $val ) . '" placeholder="&nbsp;">';
					$output .= '<span class="label sm-txt-bold">' . $placeholder . '</span>';
				...
				case "info":
					...
					if ( isset($value['name']) ) {
						$output .= '<h3 class="text-center title-group rounded-bottom mb-2 heading">' . esc_html( $value['name'] ) . '</h3>' . "\n";
					}
					if ( isset( $value['desc'] ) ) {
						$output .= $value['desc'] . "\n";
					}
				...
				case "heading":
					...
					$output .= '<div id="tab' . $counter . '" class="group ak-table ' . $class . '">';
					$output .= '<div class="content-group">';
					// $output .= '<h3 class="title-group">' . esc_html( $value['name'] ) . '</h3>' . "\n";
					break;
				...
				case "onepage":
					if( !empty( $settings['parallax_section'] ) ):
						foreach( $settings['parallax_section'] as $i => $ival ): 
							$background = $val;
							$output .='<div class="sub-option clearfix" data-id="'.$i.'">';
							
								$output .='<h3 class="title">'.__( 'Título de la página: ', 'onepage-theme' ).'<span></span><div class="section-toggle"><i class="fas fa-chevron-down"></i></div></h3>';
								
								$output .='<div class="sub-option-inner" style="display:none">';

									// Delete Section
									$output .= '<div class="wrap-btn-delete">';
									$output .= '<div class="standard-btn">';
									$output .= '<a href="javascritp:;" class="remove-parallax btn-content">'.__( 'Eliminar', 'onepage-theme' ).'</a>';
									$output .= '</div>';
									$output .= '</div>';

									// Selec page
									$output .='<div class="inline-label pt-3">';
									$output .='<label>'.__( 'Página', 'onepage-theme' ).'</label>';
									...
									$output .='<label>'.__( 'Diseño', 'onepage-theme' ).'</label>';
									$output .= '<select class="of-input of-section of-section-layout" name="' . esc_attr( $option_name . '[' . $value['id'] . ']['.$i.'][layout]' ) . '">';
									...
									$output .='<label>'.__( 'Categoría', 'onepage-theme' ).'</label>';
				...
				case "button":
					$output .= '<div class="standard-btn">';
					$output .= '<a id="' . esc_attr( $value['id'] ) . '" class="btn-content" href="javascript:void(0);">'.__( 'Añadir nueva sección', 'onepage-theme' ).'</a>'."\n";
					$output .= '</div>';
					break;
			}
			...
			if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) ) {
				$output .= '</div>';
				if ( ( $value['type'] != "checkbox" ) && ( $value['type'] != "editor" ) && ( $value['type'] != "onepage" )) {
					// $output .= '<div class="explain">' . wp_kses( $explain_value, $allowedtags) . '</div>'."\n";
				}
				$output .= '</div></div>'."\n";
			}
